...
class tmCombineDataCardInterface:
    def __init__(self, list_signalBinLabels, list_backgroundProcessLabels, list_signalProcessLabels, list_systematicsLabels, dict_observedNEvents, dict_expectedNEvents, list_rateParamLabels, dict_rateParamProperties, dict_systematicsTypes, dict_systematics):
        ...
        for rateParamLabel in list_rateParamLabels:
            ...
            if not(dict_rateParamProperties[rateParamLabel][0] in list_signalBinLabels): sys.exit("ERROR: dict_rateParamProperties[\"{rPL}\"][0] needs to be in the list of signal bin labels. Currently it is: {v}".format(v=dict_rateParamProperties[rateParamLabel][0]))
            if not((dict_rateParamProperties[rateParamLabel][1] in list_backgroundProcessLabels) or (dict_rateParamProperties[rateParamLabel][1] in list_signalProcessLabels)): sys.exit("ERROR: dict_rateParamProperties[\"{rPL}\"][1] needs to be in the list of background process labels or signal processs labels. Currently it is: {v}".format(v=dict_rateParamProperties[rateParamLabel][1]))
    ...
    def generateRateParamsSection(self):
        ...
        rateParamProcessesColumnWidth = 4 + max(max(len(processLabel) for processLabel in self.signalProcessLabels), max(len(processLabel) for processLabel in self.backgroundProcessLabels))
        ...
        for rateParamLabel in self.rateParamLabels:
            ...
            rateParamsLine += tmGeneralUtils.alignFixedWidthStringLeft(width=rateParamBinsColumnWidth, inputString=paramProperties[0])
            rateParamsLine += tmGeneralUtils.alignFixedWidthStringLeft(width=rateParamProcessesColumnWidth, inputString=paramProperties[1])
            rateParamsLine += tmGeneralUtils.alignFixedWidthFloatLeft(width=rateParamProcessesColumnWidth, precision=3, number=paramProperties[2])
            if (len(paramProperties) == 5):
                rateParamsLine += tmGeneralUtils.alignFixedWidthFloatLeft(width=rateParamProcessesColumnWidth, precision=3, number=paramProperties[3]) # initial value
                rateParamsLine += tmGeneralUtils.alignFixedWidthFloatLeft(width=rateParamProcessesColumnWidth, precision=3, number=paramProperties[4]) # final value
            yield(rateParamsLine)
...
def tmCombineDataCardInterfaceTest():
    ...
    rateParamProperties = {
        "rate1": ("signalBinTest1", "testBackground1", 1.0),
        "rate2": ("signalBinTest2", "testBackground2", 0.0, -5.0, 5.0)
    }
